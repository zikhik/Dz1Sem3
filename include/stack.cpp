	...
	auto stack<T>::top() const -> const T& { /*strong*/
	    if (empty_()) throw std::range_error("Stack is empty!");
	...
	stack<T>& stack<T>::operator=(const stack<T> &tmp) { /*strong*/
	    if (this != &tmp) { (stack(tmp)).swap(*this); }
	    return *this;
	}
	template <typename T>
	stack<T>::stack(const stack &tmp):allocator<T>(tmp.size) /*noexcept*/
	{
	    for (size_t i=0; i<tmp.count; i++) construct(allocator<T>::ptr_+i, tmp.ptr_[i]);
